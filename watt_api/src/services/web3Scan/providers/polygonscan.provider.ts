...
export class PolygonscanProvider implements IProvider {
    constructor(
    ...
    public async getAddressTransactionList(address: string, page: number, limit: number): Promise<Result<IGetTransactionListData | undefined, Error | undefined>> {
        const url = this.makeUrl(`/?module=account&action=txlist&address=${address}&page=${page}&offset=${limit}&apikey=${this.configService.props.polygonscan.key}`);
    ...
    public async getAddressTransactionListERC20(address: string, contract: string, page: number, limit: number): Promise<Result<IGetTransactionListData | undefined, Error | undefined>> {
        const url = this.makeUrl(`/?module=account&action=tokentx&address=${address}&contractaddress=${contract}&page=${page}&offset=${limit}&apikey=${this.configService.props.polygonscan.key}`);
        const log = await this.requestLogService.create('get', url);
        const before = new Date().getTime();
    ...
    public async getTransactionStatus(hash: string): Promise<Result<IGetTransactionStatusData | undefined, Error | undefined>> {
        const url = this.makeUrl(`/?module=transaction&action=gettxreceiptstatus&txhash=${hash}&apikey=${this.configService.props.polygonscan.key}`);
        const log = await this.requestLogService.create('get', url);
        const before = new Date().getTime();
    ...
    private makeUrl(url: string): string {
        return `${this.configService.props.polygonscan.url}${url}`;
    }
}