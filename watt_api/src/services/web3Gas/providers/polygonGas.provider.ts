...
export class PolygonGasProvider implements IProvider {
    constructor(
    ...
    public async getGasPrice(): Promise<Result<IGetGasPriceData | undefined, Error | undefined>> {
        ...
        try {
            ...
            const getGasPriceData: IGetGasPriceData = {
                ...
                low: evaluate(`${data.safeLow} * 1000000000`).toString()
            };
    ...
    private makeUrl(url: string): string {
        return `${this.configService.props.polygongasstation.url}${url}`;
    }
}